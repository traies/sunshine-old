...
void MouseShadowController::UpdateState(InputCommand command, HWND * wnd, int windowsCount)
{
	...
	lastXRelative = command.val1 - lastX;
	lastYRelative = command.val2 - lastY;
	...
	// Raw Input 
	ORIGINAL_WND_PROC_MULTIPLE(wnd, windowsCount, DV_INPUT, RIM_INPUT, MAKELPARAM(128, 0)); // RAW INPUT HANDLE
}
